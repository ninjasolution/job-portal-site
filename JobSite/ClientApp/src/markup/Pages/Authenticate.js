...
import { loginAction, loginConfirmedAction, signupAction } from '../../store/actions/AuthActions';
...
const Authenticate = () => {
	...
	const onSuccess = (res) => {
		saveTokenInLocalStorage(res.profileObj);
		runLogoutTimer(
			dispatch,
			3600 * 1000,
			history,
		);
		dispatch(loginConfirmedAction(res.profileObj));
		history.push('/');
	};
